...
class RocksSortedKvAdapter(SortedKvAdapter):
    env_lock = threading.Lock()
    env_dict = dict()
    count_dict = dict()
    ...
    def __init__(self, options):
        ...
        """
        with RocksSortedKvAdapter.env_lock:
            super().__init__(options)
            self.path = options["path"]
            ...
            if self.path not in RocksSortedKvAdapter.env_dict:
                if opts.create_if_missing:
                    os.makedirs(self.path, exist_ok=True)
                self.db = rocksdb.DB(self.path, opts)
                LOGGER.info("path not in dict db path:{}".format(self.path))
                RocksSortedKvAdapter.count_dict[self.path] = 0
                RocksSortedKvAdapter.env_dict[self.path] = self.db
            else:
                LOGGER.info("path in dict:{}".format(self.path))
                self.db = RocksSortedKvAdapter.env_dict[self.path]
        RocksSortedKvAdapter.count_dict[self.path] = RocksSortedKvAdapter.count_dict[self.path] + 1
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        with RocksSortedKvAdapter.env_lock:
            if self.db:
                count = RocksSortedKvAdapter.count_dict[self.path]
                if not count or count - 1 <= 0:
                    del RocksSortedKvAdapter.env_dict[self.path]
                    del RocksSortedKvAdapter.count_dict[self.path]
                else:
                    RocksSortedKvAdapter.count_dict[self.path] = RocksSortedKvAdapter.count_dict[self.path] - 1
        self.close()
        ...
        with RocksSortedKvAdapter.env_lock:
            if self.db:
                ...
                if not count or count - 1 <= 0:
                    ...
                    RocksSortedKvAdapter.count_dict[self.path] = count - 1
